...
## ۱۲) پروتکل کیفیت کد (قوانین جلسه)
12.1. **ایده و نوآوری**: قبل از هر فاز یا بلوک بزرگ، ایده‌های نوآورانه (UX، معماری، تجربه کاربر) پیشنهاد شود.
12.2. **کد کامل و تمیز**: بهترین و کامل‌ترین نسخه کد ارسال شود — TypeScript strict، error handling کامل، loading states، skeleton، accessibility.
12.3. **بدون کامنت‌های `///`**: از تگ‌های هایلایتی (مثل `///` یا `/***/`) خودداری شود. فقط کامنت‌های ضروری با `//` یا `/* */` که واقعاً توضیح لازم دارند. کد باید خودش حرف بزند.
12.4. **نام‌گذاری گویا**: متغیرها، توابع و کامپوننت‌ها باید اسمشان کارشان را توضیح دهد تا نیاز به کامنت کم شود.

## ۱۳) درس‌های فاز 4B (سفارش و پرداخت)
15. redirect() هرگز داخل try/catch گذاشته نشود؛ exception داخلی NEXT_REDIRECT بلعیده می‌شود و به‌جای پریدن، مسیر خطا اجرا می‌شود.
16. cookies().set() فقط در Server Action یا Route Handler مجاز است؛ برای رندر صفحه از نسخه فقط‌خواندنی (findUnique + بازگشت خالی) استفاده کن.
17. طراحی با استعاره واقعی (کاغذ، مهر چرخان، خط برش، بارکد) به‌جای کارت جنریک؛ جزئیات کوچک مثل هم‌پوشانی مهر روی متن، حس اصالت می‌سازد.
18. select کنترل‌نشده با defaultValue بعد از server action به‌روز نمی‌شود؛ با key={مقدار} آن را remount کن.
19. هر بخش جدید ادمین = لینک در sidebar؛ کاربر نباید URL را دستی تایپ کند. badge تعداد pending هم سیگنال عملیاتی می‌دهد.
